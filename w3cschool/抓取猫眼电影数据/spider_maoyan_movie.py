...
class Spider:
    ...
    def get_film_info(self, page=100):
        ...
        while True:
            ...
            if i < 100 and flag:
                res = requests.get(self.url.format(i * 30),
                                   headers=self.get_headers(False))
            else:
                res = requests.get(self.url.format(i * 30),
                                   headers=self.get_headers(True))
            soup = BeautifulSoup(res.text, 'lxml')
            temp1 = soup.find_all('div', attrs={
            ...
            if len(temp1) < 1:
                ...
                i += 1
                continue
            temp2 = soup.find_all('div', attrs={
                'class': 'channel-detail channel-detail-orange'})
            error_num2 = 0
            for t1, t2 in zip(temp1, temp2):
                ...
                if t2.string:
                    ...
                    try:
                        score = t2.find_all('i')[0].string.strip() + \
                                t2.find_all('i')[1].string.strip()
                    except:
                        score = 0
                ...
                if not temp:
                    ...
                    # temp = soup.find('span', attrs={'class': "dra"})
                try:
                    introduction = temp.string.strip()
                    error_num2 = 0
                except:
                    error_num2 += 1
                    introduction = '暂无'
                films.append([name, score, link, introduction])
            i += 1
            error_num = 0
        return films
    ...
    def get_cookies(self, login=False):
        return cookies.login_Cookies[random.randint(0, len(
            cookies.login_Cookies) - 1)] if login else \
