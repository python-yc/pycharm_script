...
class GetNameId:
    def get_id(self, song_name):
        ...
        # 切换
        self.framel = self.browser.find_element_by_css_selector(".g-iframe")
        self.browser.switch_to.frame(self.framel)
        # 定位
        ...
        self.browser.close()
        self.pattern = re.compile(r'<div cla.*?song(.*?)" cla.*?b title="(.*?)">.*?artist.*?\d+">([^<][^s].*?)</.*?album.*?le="(.*?)">.*?</div>')
        # re.S用于跨行匹配
